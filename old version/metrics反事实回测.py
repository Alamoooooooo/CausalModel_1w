...
# V4 反事实策略回测（Counterfactual Policy Evaluation）

> 目标：不依赖“真实发生结果”，而是估计：
> **如果当时采用该推荐策略，理论上能带来多少收益？**

这是更因果严谨的：
- 离线策略价值评估（Offline Policy Evaluation, OPE）
- 反事实收益估计
...
- `Y`：真实结果
- `cate`：模型估计的个体增量
- `ps`：propensity score（被触达概率）
- `reco_flag`：策略是否推荐（0/1）
...
# 示例：Top-K推荐作为策略
K = int(len(recommend_df) * 0.2)
topk = recommend_df.sort_values("cate_rank").head(K)

topk["reco_flag"] = 1
cf_df = cf_df.merge(
    topk[["cust_id","product_id","reco_flag"]],
    on=["cust_id","product_id"],
    how="left"
)
cf_df["reco_flag"] = cf_df["reco_flag"].fillna(0)
```
...
```python
# 避免极端权重
cf_df["ps_clip"] = cf_df["ps"].clip(0.01, 0.99)

# IPW估计策略价值
cf_df["ipw_weight"] = (
    cf_df["reco_flag"] * cf_df["T"] / cf_df["ps_clip"]
)

ipw_value = (cf_df["ipw_weight"] * cf_df["Y"]).sum() / len(cf_df)
print("IPW Policy Value:", ipw_value)
```
...
cf_df["dr_term"] = (
    cf_df["reco_flag"] * (
        cf_df["mu1"] - cf_df["mu0"]
        + cf_df["T"] * (cf_df["Y"] - cf_df["mu1"]) / cf_df["ps_clip"]
        - (1 - cf_df["T"]) * (cf_df["Y"] - cf_df["mu0"]) / (1 - cf_df["ps_clip"])
    )
)


dr_value = cf_df["dr_term"].mean()
print("DR Policy Value:", dr_value)
```
...
# 随机策略
np.random.seed(42)
cf_df["rand_flag"] = np.random.binomial(1, cf_df["reco_flag"].mean(), len(cf_df))

cf_df["rand_weight"] = cf_df["rand_flag"] * cf_df["T"] / cf_df["ps_clip"]
rand_value = (cf_df["rand_weight"] * cf_df["Y"]).sum() / len(cf_df)

print("Model Policy Value:", dr_value)
print("Random Policy Value:", rand_value)
print("Lift:", dr_value - rand_value)
```
...
ROI Engine（是否赚钱）
   ↓
Counterfactual Policy Value（理论可得收益）
```
