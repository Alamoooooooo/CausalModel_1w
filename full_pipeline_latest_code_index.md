...
本文只收录“最新版本”的主流程代码，并按推荐执行顺序排列。  
目标是把全流程中最核心、最常用、最应该优先阅读的代码完整贴在一个文件里，便于快速查阅与排障。

---

## 1. `src/bundle_mining_pipeline.py`

### 作用
- bundle / mining 主流程
- 负责特征挖掘、bundle 相关数据准备、上游产物生成
...
"""
这里请粘贴 `src/bundle_mining_pipeline.py` 的完整最新代码。
"""
...
## 2. `src/bundle_cate_train_pipeline_v3.py`
...
"""
这里请粘贴 `src/bundle_cate_train_pipeline_v3.py` 的完整最新代码。
"""
...
## 3. `src/backtest_full_pipeline_v3.py`
...
- 负责：
  ...
  - temporal 稳定性
  - OPE 降级/评估
  - single_day / full / both 三种模式
- 我之前已为它补充了文件尾端 CLI 注释说明

### 对应说明
- `backtest_full_pipeline.md`
- `backtest_full_pipeline_functions.md`
